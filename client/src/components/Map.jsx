...
const Map = () => {
    ...
    const [forecast, setForecast] = useState({}); // [latitude, longitude

    const [farms, setFarms] = useState([])

    const getWeather = () => {
        axios.get(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&exclude=hourly,daily&appid=${openweatherKey}`)
            .then(response => {
        ...
        <div className="map" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: '90vh' }} >
            ...
            <div className="content" style={{ width: '60rem' }}>
                ...
                <div className="weather">
                    ...
                    <p>Temperature: {forecast.current && forecast.current.temp}°C</p>
                    <p>Humidity: {forecast.current && forecast.current.humidity}%</p>
                    <p>Wind Speed: {forecast.current && forecast.current.wind_speed}m/s</p>
                    <p>Weather: {forecast.current && forecast.current.weather[0].description}</p>
                </div>
                {/* Tooltip content */}
                {farms.length != 0 && farms.map((farm) => {
                    return (
